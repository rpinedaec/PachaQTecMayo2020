-- MySQL Workbench Synchronization
-- Generated: 2020-07-04 12:31
-- Model: New Model
-- Version: 1.0
-- Project: Name of the project
-- Author: Ricardo Avellaneda

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

CREATE SCHEMA IF NOT EXISTS `jacaton6avellaneda` DEFAULT CHARACTER SET utf8 ;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`empresa` (
  `idempresa` INT(11) NOT NULL AUTO_INCREMENT,
  `rucEmpresa` VARCHAR(45) NOT NULL,
  `nombreEmpresa` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idempresa`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`tipoPago` (
  `idtipoPago` INT(11) NOT NULL AUTO_INCREMENT,
  `descTipoPago` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idtipoPago`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`clientes` (
  `idclientes` INT(11) NOT NULL AUTO_INCREMENT,
  `nombreCliente` VARCHAR(45) NOT NULL,
  `numeroIdCliente` VARCHAR(45) NOT NULL,
  `direccionCliente` VARCHAR(200) NOT NULL,
  PRIMARY KEY (`idclientes`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`facCabecera` (
  `idfacCabecera` INT(11) NOT NULL AUTO_INCREMENT,
  `idempresa` INT(11) NOT NULL,
  `idtipoPago` INT(11) NOT NULL,
  `idclientes` INT(11) NOT NULL,
  `fechaFacCabecera` DATETIME NOT NULL,
  `igvFacCabecera` DECIMAL NOT NULL,
  `subtotalFacCabecera` DECIMAL NOT NULL,
  `totalFacCabecera` DECIMAL NOT NULL,
  `estadoFactura` CHAR(1) NOT NULL,
  PRIMARY KEY (`idfacCabecera`),
  INDEX `fk_facCabecera_tipoPago1_idx` (`idtipoPago` ASC),
  INDEX `fk_facCabecera_empresa1_idx` (`idempresa` ASC),
  INDEX `fk_facCabecera_clientes1_idx` (`idclientes` ASC),
  CONSTRAINT `fk_facCabecera_tipoPago1`
    FOREIGN KEY (`idtipoPago`)
    REFERENCES `jacaton6avellaneda`.`tipoPago` (`idtipoPago`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_facCabecera_empresa1`
    FOREIGN KEY (`idempresa`)
    REFERENCES `jacaton6avellaneda`.`empresa` (`idempresa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_facCabecera_clientes1`
    FOREIGN KEY (`idclientes`)
    REFERENCES `jacaton6avellaneda`.`clientes` (`idclientes`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`productos` (
  `idproductos` INT(11) NOT NULL AUTO_INCREMENT,
  `nombreProducto` VARCHAR(45) NOT NULL,
  `valorProducto` DECIMAL NOT NULL,
  `igvProducto` TINYINT(4) NOT NULL,
  PRIMARY KEY (`idproductos`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `jacaton6avellaneda`.`facDetalle` (
  `idfacDetalle` INT(11) NOT NULL AUTO_INCREMENT,
  `idfacCabecera` INT(11) NOT NULL,
  `idproductos` INT(11) NOT NULL,
  `cantFacDetalle` INT(11) NOT NULL,
  `valorFacDetalle` DECIMAL NOT NULL,
  PRIMARY KEY (`idfacDetalle`),
  INDEX `fk_facDetalle_facCabecera_idx` (`idfacCabecera` ASC),
  INDEX `fk_facDetalle_productos1_idx` (`idproductos` ASC),
  CONSTRAINT `fk_facDetalle_facCabecera`
    FOREIGN KEY (`idfacCabecera`)
    REFERENCES `jacaton6avellaneda`.`facCabecera` (`idfacCabecera`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_facDetalle_productos1`
    FOREIGN KEY (`idproductos`)
    REFERENCES `jacaton6avellaneda`.`productos` (`idproductos`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
