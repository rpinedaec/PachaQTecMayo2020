...
"""@app.route('/Detalle/add/<cliente>', methods=['GET', 'POST'])
def addDetalle(cliente='hola'):
    form = AddFacturaForm()
    form1 = AddDetalleFacturaForm()
    #cliente = Factura.query.filter_by(cliente=form.cliente.data).order_by(Factura.id.desc()).first()
    if form1.validate_on_submit():
        detalle = DetalleFactura(factura=form.id.data, producto=form1.producto.data, cantidad=5)
        db.session.add(detalle)
        db.session.commit()
        flash('Factura Realizada!')
        return redirect(url_for('getFacturas'))
    return render_template('AddDetalle.html', title='Detalle Factura', cliente=cliente, form1=form1)"""