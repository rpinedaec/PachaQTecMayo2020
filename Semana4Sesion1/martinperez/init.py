...
def cargaInicial():
    ...
    # cargar el archivo en las clases y 
    # cargar el listado
    try:    
        res = fileProducto.leerArchivo()
        log.debug(res)
        lstProductos = json.loads(res)
        #log.debug(type(lstProductos))
        for dicProducto in lstProductos:
            #log.debug(dicProducto["nombreProducto"])
